...
% input:  Imap    ---  quantized image
%         ImgS    ---  segments
%         outseg -- number of segments you want
% output: I       ---  final image

% JSEG; 4.3 Merge regions, using Matlab linkage for merging
% every time merge two regions, region number reduces one
%% rewrite by Minjie
IRGB = double(IRGB);
segnum = max(ImgS(:));
qlv =  max(Imap(:));
